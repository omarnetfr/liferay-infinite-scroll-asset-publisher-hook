...
<liferay-ui:tabs
	formName="fm"
	names="asset-selection,display-settings,subscriptions,infinite-scroll"
	param="tabs2"
	refresh="<%= false %>"
>
	<liferay-ui:section>
	...
	<liferay-ui:section>		
		<aui:input label='<%= LanguageUtil.format(pageContext, "enable-inifinite-scroll", false) %>' name="preferences--enableInfiniteScroll--" type="checkbox" value="<%= enableInfiniteScroll %>" />
		
		<aui:input label='<%= LanguageUtil.format(pageContext, "container-height", false) %>' name="preferences--containerHeight--" type="text" value="<%= containerHeight %>" />
		
		<aui:input label='<%= LanguageUtil.format(pageContext, "container-selector", false) %>' name="preferences--containerSelector--" type="text" value="<%= containerSelector %>" />
		
		<aui:input label='<%= LanguageUtil.format(pageContext, "enable-debug", false) %>' name="preferences--enableDebug--" type="checkbox" value="<%= enableDebug %>" />
		
		<%-- <aui:input label='<%= LanguageUtil.format(pageContext, "styles", false) %>' name="preferences--styles--" type="text" value="<%= styles %>" /> --%>
		
		<aui:select label="link-window-state" name="preferences--linkWindowState--">
			<aui:option label="minimized" selected="<%= linkWindowState.equals(WindowState.MINIMIZED.toString()) %>" value="<%= WindowState.MINIMIZED.toString() %>" />
			<aui:option label="normal" selected="<%= linkWindowState.equals(WindowState.NORMAL.toString()) %>" value="<%= WindowState.NORMAL.toString() %>" />
			<aui:option label="maximized" selected="<%= linkWindowState.equals(WindowState.MAXIMIZED.toString()) %>" value="<%= WindowState.MAXIMIZED.toString() %>" />
		</aui:select>
		
		<aui:input type="hidden" name="preferences--templateItem--" value="<%= templateItem %>" />
		
		<label><%= LanguageUtil.format(pageContext, "template-item", false) %></label>
		<div class="lfr-editor-container" id="<portlet:namespace />templateContainer">
			<div class="lfr-rich-editor" id="<portlet:namespace />templateRichEditor"></div>
		</div>
...
<aui:button-row>
	<aui:button onClick='<%= renderResponse.getNamespace() + "saveItemTemplate();" + renderResponse.getNamespace() + "saveSelectBoxes();" %>' type="submit" />
</aui:button-row>

<aui:script use="aui-base,aui-ace-editor">

	var editorContentElement = A.one('#<portlet:namespace />templateItem');
	
	var editorNode = A.one('#<portlet:namespace />templateRichEditor');
	...
	Liferay.provide(
		window,
		'<portlet:namespace />saveItemTemplate',
		function() {
			editorContentElement.val(getEditorContent());
		}
	);
